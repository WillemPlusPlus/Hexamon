...
export class SpriteController {
    ...
    constructor() {
        ...
        this.canvasSecret = append("body", "canvas", { class: "offscreen" });
     ...
     * @returns
     */
    getSpritePositions(base, offset, random) {
     ...
     */
    generateSprite(customSettings, idPreset = "large", idSheet = "body", indexRolls) {
        ...
        for (const [k, v] of Object.entries(preset)) {
            ...
            if (customSettings.file) {
                ...
                if (k != "eyeR") {
                    spritePart = this.sprites.get(part.sprite).getRandom(isPair, partIndex[k]);
                }
                else {
                    index = 1;
                }
...
export class Sprite {
    ...
    layout;
    partIndex;
    constructor(id, image, idSheet, indexs) {
        //A canvas for image manipulation
        this.canvasSecret = append("body", "canvas", { class: "offscreen" });
        this.canvasSecret.style.display = "none";
        this.id = id;
     ...
     */
    draw(ctx, pos, scale, index = 0, scaleAbsolute = false) {
        let ix;
    ...
    getPart(id) {
        ...
        let size = DATA.customOrder[0].size;
    ...
    getRandom(isPair, index) {
        ...
        let ctx = this.canvasSecret.getContext('2d');
        ctx.clearRect(0, 0, this.layout.w, this.layout.h);
        ctx.drawImage(this.image, x * this.layout.w, y * this.layout.h, this.layout.w, this.layout.h, 0, 0, this.layout.w, this.layout.h);
        if (isPair) {
...
export class Img {
    ...
    importData(imgData) {
        this.colours.forEach((col, coli) => {
            const colour1d = imgData.data.filter((data, datai) => datai % 4 == coli);
     ...
     */
    pasteData(imgData, positions, onTop = true) {
        const colours1d = this.colours.map((c, i) => imgData.data.filter((data, datai) => datai % 4 == i));
     ...
     */
    static hsvToRgb(hsv, hsvMod = { h: 0, s: 0, v: 0 }) {
        let h, s, v;
