...
import { rand, randElement, range } from "../utils.js";
...
export class GameController {
    ...
    constructor(controller, inject, injectChallenge) {
    ...
    onResize = () => {
        if (!this.ctx)
            throw "Cannot resize CTX is null";
    ...
    update() {
        ...
        for (let [id, creature] of this.zSort(this.creatures)) {
            if (!creature.isAlive) {
                ...
                ;
                if (this.injectChallenge && this.injectChallenge.creatureTick)
                    this.injectChallenge.creatureTick(creature, this.dt);
        ...
        for (let [id, hex] of this.zSort(this.hexTiles)) {
            if (this.injectChallenge && this.injectChallenge.hexTick)
                this.injectChallenge.hexTick(hex, this.dt);
            if (this.defaultInstructions)
                hex.growFood(this.dt);
        }
    }
    /**
     * Update screen
     ...
     */
    maybeMoveCreature(creature, chance) {
     ...
     */
    spawnCreature(hexParent, range = 2, lvl = 1) {
    ...
    */
    ...
    createBoardCustom(props) {
        ...
        for (const creatureProps of creatures) {
            ...
            if (!hex)
                throw "No hex found at row: " + creatureProps.pos.row + " col: " + creatureProps.pos.col;
            ...
            if (creature.species.instructionsStart) {
                creature.species.instructionsStart(creature);
            }
            this.creatures.set(creature.id.toString(), creature);
        }
        ...
        for (const foodProps of food) {
            ...
            if (!hex)
                throw "No hex found at row: " + foodProps.row + " col: " + foodProps.col;
    ...
    elevateBiomes() {
        for (let hex of [...this.hexTiles.values()]) {
            if (hex.biome == "grass" && this.elevateNeigbors(hex.row, hex.col, ["grass"])) {
                hex.setBiome("lush");
            }
            if (hex.biome == "rock" && this.elevateNeigbors(hex.row, hex.col, ["rock", "cliff"])) {
                hex.setBiome("cliff");
            }
            if (hex.biome == "sand" && this.elevateNeigbors(hex.row, hex.col, ["sand"])) {
                hex.setBiome("lush");
            }
            if (this.elevateNeigbors(hex.row, hex.col, ["ocean"])) {
                hex.setBiome("ocean");
            }
        }
    }
     ...
     * @returns Hex Tile
     */
    getRandomHex(filter = (candidate) => true) {
     ...
     */
    createCreatureAt(x, y, isCustom) {
        let hex;
        if (x == -1 && y == -1) {
            hex = this.getRandomHex((candidate) => !candidate.hasCreature && candidate.inArena);
        }
        else {
     ...
     */
    createCreature(hex, species, lvl, settings) {
