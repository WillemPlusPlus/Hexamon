...
import { UIController } from "./control/UIController.js";
import { Sprite, Img } from "./control/SpriteController.js"
import ANIMS from "./data/animations.js";
import { dist,copy,rand} from "./utils.js";
import {species} from "./species/species.js";
import { Stats, StatName, Pos} from "./data/types.js";
...
lvlXP: number;
age: number;
positionSprite: {x: number, y: number}

hex: Tile;
sprite: string;
controller: GameController;
translation?: {x:any[], y:any[]}
spriteSheetIndex: number;
type: string;
typeId: number;
route: {start: Tile, end: Tile, distance: number, distanceTotal: number}
velocity =  { dx: 0 ,dy: 0 };



stateID = CONFIG.creature.stateDefault as string; 
 ...
 */
static generateCurrentStats(lvl: number, statsRatio: Stats): Stats{
 ...
 */
static generateStatRatio(statsTemplate:Stats):Stats{
 ...
 */
static generateStats(n:number): Stats{
    let stats = {};
 ...
 */
static isValidMove(rowD, colD, debug = true):boolean{
    ...
    if(debug)console.log(rowD+" , " + colD + " is not a valid move.")
    return false

}

constructor(controller: GameController, hex: Tile, speciesID: string = "", lvl = 1, settings = {hasHP:true, hasNRG:true, hasXP: true, hasDrops:true}){
    ...
    if(settings){
        ...
        if(this.hasXP  && speciesID != "enemy")this.bars.push("xp")
        if(this.hasHP  && speciesID != "enemy")this.bars.push("hp")
        if(this.hasNRG && speciesID != "enemy")this.bars.push("nrg")
    }
...
get target(){
    if(!this.isAttacking)return undefined
 ...
 * @param mod number
 */
modifyStat(stat:StatName, mod:number){
    if(stat == "xp")this.xp += mod
    else this.stats.mods[stat] += mod
}

boostStat(stat:StatName, mod:number){
    if(!this.stats.boosts[stat]){console.log(stat ," is not a recognised stat name")
    }else{
...
applyTranslations(position, filter = "", autoKeep = false){
    ...
    for(let axis of ["x", "y"]){
        ...
        while(pop){
            if((!pop.isTemporary||autoKeep) || (filter && pop.source != filter))translationsPermanent[axis].push(pop);
            if(!filter || (filter && pop.source == filter))translations[axis] += pop.value
 ...
 */
update(dt: number, isUpdateFrame: boolean) {
    if(isUpdateFrame){
...
triggerTrait(trigger:string, them?:Creature){

    for(let traitName of this.species.traits){
        let trait = traits.find((t)=>t.name == traitName)
        if(!trait){throw "Trait" + traitName+" does not exist"}
...
callEffect(effect, them?:Creature){
...
calculateEffects(dt){
    ...
    for(let effect of this.effectSources){
        if(effect.trait.step == "everyFrame")callEffect = true
        if(effect.trait.step == "every12Frames"&&this.cooldownFast<=0)callEffect = true
        if(effect.trait.step == "every60Frames"&&this.cooldownMedium<=0)callEffect = true
        if(effect.trait.step == "every300Frames"&&this.cooldownSlow<=0)callEffect = true
        if(typeof effect.trait.step == "function"&& effect.trait.step(this))callEffect = true
        if(callEffect){this.callEffect(effect, this)}
    }
    ...
    if(this.cooldownFast<=0)this.cooldownFast = 0.2
    if(this.cooldownMedium<=0)this.cooldownMedium = 1
    if(this.cooldownSlow<=0)this.cooldownSlow = 5
}
 ...
 */
dealDamage(hex: Tile){
    if(!hex.hasCreature) return;
    if(hex.creature.dodgeDamage()) return
...
metabolise(dt: number){
    ...
    if(this.hasXP && this.xp>=this.lvlXP){
        if(this.lvl == POKEDEX.stats.lvlMax){this.xp=this.lvlXP}
        else{
        this.lvlUp();
        }
    }
...
lvlUp(){
    ...
    if(this.lvl>=POKEDEX.stats.lvlBreakPoints[this.age]){
        ...
        this.age++
        let sprite = this.controller.get("sprite#"+this.sprite) as Sprite
...
die() {
    ...
    if(this.species.reproduction == "semelparity" && this.isFertile){this.semelparity()}
    if(this.hasDrops)this.hex.spawnFood(3*this.lvl/100, "red")
 ...
 * @returns 
 */
setMitosis(hexNew: Tile){
    if(hexNew.hasCreature)return;
    //clear old tile
    let hexOld = this.hex
 ...
 * @returns boolean success
 */
moveRelative(rowD: number, colD: number, debug = false){
    if(!this.readyToMove)return false
    if(Creature.isValidMove(rowD, colD, debug)){
        let hexNew = this.controller.getHex(this.hex.row+rowD, this.hex.col+colD)
        if(!hexNew){
            if(debug)console.log("Destination hex does not exist")
            return false
        }
        if(!hexNew.inArena){
            if(debug)console.log("Destination hex is not in arena")
            return false
        }
...
moveToHex(hex: Tile) {
    if (hex.hasCreature){
        if(hex.creature.isAttacking || hex.creature.isMoving)return false;
...
animate( dist , animation){
    ...
    const startTimedAnimation = () => {
        return sequence.reduce((steps, stepCurr, i)=>{
            return steps.then(()=>{
                applyNextFrame(i);
                return new Promise<void>(resolve=>{setTimeout(()=>resolve(), stepCurr.duration*tFac)})
            })},Promise.resolve())
    }
    /**
 ...
 * @returns 
 */
drawName(ctx:CanvasRenderingContext2D, position){
 ...
 * @param position anchor Pos
 */
drawIcon(ctx:CanvasRenderingContext2D, stat:StatName, position){
    let props = {
        x: position.x,
        y: position.y,
    ...
    if(stat=="xp"){
        ...
        let icons = this.controller.get("sprite#icons") as Sprite
 ...
 * @param position 
 */
drawIndicator(ctx:CanvasRenderingContext2D, stat: StatName, position){
 ...
 * @param position anchor Pos
 */
drawBar(ctx:CanvasRenderingContext2D, stat:StatName, position){
    let baseline = stat == "xp"?this.lvlXP:this.stats.base[stat]
    let statNorm = this.getStat(stat)/baseline;
    statNorm = Math.min(statNorm, 1);
 ...
 * @param size scale
 */
draw(ctx: CanvasRenderingContext2D, offset = {x:0,y:0}, size = this.size) {
    let sprite = this.controller.get("sprite#"+this.sprite) as Sprite
    let position = {
