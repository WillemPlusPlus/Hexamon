...
import { Sprite } from "./control/SpriteController";
import { createUnparsedSourceFile } from "../../node_modules/typescript/lib/typescript.js";
...
hasFriend(spec){
  ...
  growFood(dt){
    ...
    let amount = 0
    if(roll<chance){
      if(roll<chance/5){amount++}
      if(roll<chance/25){amount++}
      let colour = "green"
      if((this.biome == "rock" || this.biome == "water") && Math.random()<0.5){colour = "red"}
      if(this.biome == "sand"){colour == "red"}
      this.spawnFood(amount, colour)
    }
  }

  spawnFood(amount = 1, colour="green"){
    if(this.hasFood){this.food.combine(colour, amount)}
    else{
  ...
  setBiome(name){
    if(name == "ocean"||name == "cliff"){this.inArena = false}
    this.biome = name
  }
  /**
   ...
   */
  drawText(ctx: CanvasRenderingContext2D, text: string, offset, x=this.position.x, y=this.position.y) {
    ctx.font = CONFIG.canvas.text.font;
    ctx.textAlign = CONFIG.canvas.text.align as CanvasTextAlign;
    ctx.textBaseline = CONFIG.canvas.text.baseline as CanvasTextBaseline;
    ctx.fillStyle = CONFIG.canvas.colors.text;
    ctx.fillText(text, x + offset.x, y + offset.y);
  }


  drawTexture(ctx: CanvasRenderingContext2D, x=this.position.x, y=this.position.y, clear: boolean = true){
   ...
   */
  drawHexagon(ctx: CanvasRenderingContext2D, x=this.position.x, y=this.position.y, clear: boolean = true) {
    ctx.beginPath();
    ctx.moveTo(x, y + CONFIG.hex.radius);
    for (let i = 1; i <= 6; i++) {
      ctx.lineTo(
        x + CONFIG.hex.radius * Math.sin(i * CONFIG.hex.angle),
        y + CONFIG.hex.radius * Math.cos(i * CONFIG.hex.angle)
      );
    }
    ctx.closePath();
   ...
   * @param {Boolean} clear - delete underlying?
   */
  draw(ctx: CanvasRenderingContext2D, offset = {x:0,y:0}, clear: boolean = true) {
    ...
    if(this.hasFood)this.food.draw(ctx, offset);
    if (this.inArena) {
